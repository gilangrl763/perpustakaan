...
$('.multiupload').MultiFile({
	afterFileAppend: function(element, value, master_element) {
		...
		var reqMaxId = $("#reqMaxId").val();
		var reqGenerate = $("#reqGenerate").val();
		...
		$("input[name="+variable+"]").each(function( index ) {
			if((index) == total - 2)
			{
				 ...
				 var dataid = reqMaxId ?? master_element.E.data("dataid");
...
function uploadProgress(element,index,jenisDokumen,variable,dataid)
{
	...
	var reqGenerate = $("#reqGenerate").val();
	var reqId = $("#reqId").val();
	var reqRekananId = $("#reqRekananId").val();
	var reqPaketId = $("#reqPaketId").val();
	var reqPaketRekananId = $("#reqPaketRekananId").val();
	var reqNomorPO = $("#reqNomorPO").val();
	var reqJenisPenilaian = $("#reqJenisPenilaian").val();
	...
	if(!window.File && window.FileReader && window.FileList && window.Blob){ //if browser doesn't supports File API
		...
		formData.append('reqGenerate', reqGenerate);
		formData.append('reqRekananId', reqRekananId);
		formData.append('reqPaketId', reqPaketId);
		formData.append('reqPaketRekananId', reqPaketRekananId);
		formData.append('reqNomorPO', reqNomorPO);
		formData.append('reqJenisPenilaian', reqJenisPenilaian);
		

		//jQuery Ajax to Post form data
		$.ajax({
			url : "dokumen_manager_json/upload_temporary_icmesh",
			type: "POST",
			data : formData,
			contentType: false,
			cache: false,
