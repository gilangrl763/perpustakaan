...
function uploadProgress(element,dokumenManagerId,urutDokumen,index,jenisDokumen,jenisDokumenId,namaDokumen,variable,dataid)
{
	...
	if(!window.File && window.FileReader && window.FileList && window.Blob){ //if browser doesn't supports File API
		...
		$.ajax({
			url : "uploads_json/upload_temporary_jaminan_sanggah/",
			type: "POST",
			data : formData,
			contentType: false,
			cache: false,
			...
			{
				$("#reqLinkFileJaminanSanggahTemp"+namaDokumen).val(obj.file);				
				$("#divButton"+namaDokumen).html(`<button type="button" onclick="openAdd('uploads/jaminan/`+obj.file+`')">Lihat Dokumen</button>`);
