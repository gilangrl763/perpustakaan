...
if (window.jQuery)(function ($) {
				...
				MultiFile.addToList = function (slave, slave_count, files) {
					...
					$.each(files, function (i, file) {
						var v = String(file.name || '' ),
								...
								dokid = MultiFile.E.data("dokumenid"),
								variable = MultiFile.E.data("variable"),
								urutid = MultiFile.E.data("id")+''+slave_count,
								p = '<img class="MultiFile-preview" style="'+ MultiFile.previewCss+'"/>',
								label =	$(
										(
											'<span class="MultiFile-label" title="' + t + '">'+ 
											'<input type="hidden" name="'+variable+'ManagerId[]" value="">'+ 
											'<input type="hidden" name="'+variable+'Id[]" value="'+dokid+'">'+ 
											'<input type="hidden" name="'+variable+'Slave[]" value="'+slave_count+'">'+ 
											'<input type="hidden" name="'+variable+'Temp[]" id="'+variable+'Temp'+urutid+'" value="">' + 
											'<input type="hidden" name="'+variable+'TempUkuran[]" id="'+variable+'TempUkuran'+urutid+'" value="">' + 
											'<input type="hidden" name="'+variable+'TempTipe[]" id="'+variable+'TempTipe'+urutid+'" value="">' +
											'<input type="hidden" name="'+variable+'TempNama[]" id="'+variable+'TempNama'+urutid+'" value="'+n+'">' +
											'<div id="progressBar'+urutid+'" style="display:none;">' +
											' <div class="progress">'+
											'	  <div class="progress-bar" id="progress-bar'+urutid+'" role="progressbar">'+
											'		<label id="status'+urutid+'"></label>'+
											'	  </div>'+
											' </div> ' +
											'</div>' +
					...
					//$.each(files, function (i, file) {
						...
						var urutid = MultiFile.E.data("id")+''+slave_count;
						
						var
							r = $('<div class="MultiFile-label"></div>'),
							b = $('<a id="multiFileRemove'+urutid+'" class="MultiFile-remove" href="#' + MultiFile.wrapID + '">' + MultiFile.STRING.remove + '</a>')
	...
	*/
	$.fn.MultiFile.options = { //$.extend($.fn.MultiFile, { options: {
		...
		// previews
		preview: false,
		previewCss: 'max-height:150px; max-width:150px;',

		// STRING: collection lets you show messages in different languages
		STRING: {
			remove: 'x',
			denied: 'Ekstensi *.$ext tidak diperbolehkan.',
			file: '$file',
			selected: 'File selected: $file',
			duplicate: 'Tidak dapat upload dokumen dengan nama yang sama',
			toomuch: 'File yang anda pilih melebihi batas ($size)',
			toomany: 'File yang anda pilih melebihi batas. (maksimal: $max)',
			toobig: 'Ukuran file $file terlalu besar (maksimal: $size)'
		},
