...
$('.multiupload').MultiFile({
	afterFileAppend: function(element, value, master_element) {
		...
		// alert(maxprnumber);return false;
...
{
	var reqPrNumber = $("#reqPrNumber").val();
	var reqItemNumber = $("#reqItemNumber").val();
	// alert(maxprnumber);return false;
	...
	if(!window.File && window.FileReader && window.FileList && window.Blob){ //if browser doesn't supports File API
		...
		formData.append('reqPrNumber', reqPrNumber);
		formData.append('reqItemNumber', reqItemNumber);
		

		//jQuery Ajax to Post form data
		$.ajax({
			url : "dokumen_manager_json/upload_temporary_item",
			type: "POST",
			data : formData,
			contentType: false,
			cache: false,
