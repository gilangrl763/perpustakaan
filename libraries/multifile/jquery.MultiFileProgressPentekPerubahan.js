...
$('.multiupload').MultiFile({
	afterFileAppend: function(element, value, master_element) {
		...
		var maxprnumber = $("#reqMaxPrNumber").val();
		var reqGenerate = $("#reqGenerate").val();
		var reqPrGroupNumber = $("#reqPrGroupNumber").val();
		// alert(reqGenerate);return false;
		...
		$("input[name="+variable+"]").each(function( index ) {
			if((index) == total - 2)
			{
				 ...
				 var dataid = maxprnumber ?? master_element.E.data("dataid");
...
function uploadProgress(element,index,jenisDokumen,variable,dataid)
{
	...
	var reqGenerate = $("#reqGenerate").val();
	var reqPrGroupNumber = $("#reqPrGroupNumber").val();
	// alert(reqPrGroupNumber);return false;

	// alert(dataid);return false;
	//configuration
	...
	if(!window.File && window.FileReader && window.FileList && window.Blob){ //if browser doesn't supports File API
		...
		formData.append('reqId', maxprnumber);
		...
		$.ajax({
			url : "dokumen_manager_json/upload_temporary_pentek_perubahan",
			type: "POST",
			data : formData,
			contentType: false,
			cache: false,
