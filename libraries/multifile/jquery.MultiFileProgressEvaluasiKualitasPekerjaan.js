	...
	afterFileAppend: function(element, value, master_element) {
		...
		$("input[name="+variable+"]").each(function( index ) {
			if((index) == total - 2)
			{
				 ...
				 var dataid = reqId ?? master_element.E.data("dataid");
...
function uploadProgress(element,index,jenisDokumen,variable,dataid)
{
	...
	if(!window.File && window.FileReader && window.FileList && window.Blob){ //if browser doesn't supports File API
		...
		formData.append('reqSumber', reqSumber);
		formData.append('reqGenerate', reqGenerate);
		

		//jQuery Ajax to Post form data
		$.ajax({
			url : "dokumen_manager_json/upload_temporary_evaluasi_kualitas_pekerjaan",
			type: "POST",
			data : formData,
			contentType: false,
			cache: false,
