				...
				if (data.levels) {
					$.each(data.levels, function(index, value) {
						...
						$.each(value.locations, function(index, value) {
							...
							if (value.pin != 'false') {
								if (o.locations) var pin = $('<a></a>').attr({'href': '#' + value.id, 'title': value.label}).addClass(value.icon).css({'top': top + '%', 'left': left + '%'}).appendTo(locations);											
							...
							if (o.sidebar) {
								...
								$('<div></div>').html(value.kondisi).appendTo(link).addClass(value.kondisi); 
