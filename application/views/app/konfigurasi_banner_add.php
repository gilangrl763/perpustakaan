...
if ($id == "") {
    ...
    $banner->selectByParams(array("A.BANNER_ID" => $id));
    $banner->firstRow();

    $BANNER_ID          = $banner->getField("BANNER_ID");
    $NAMA               = $banner->getField("NAMA");
    $KETERANGAN         = $banner->getField("KETERANGAN");
    $GAMBAR             = $banner->getField("GAMBAR");
    $STATUS             = $banner->getField("STATUS");
}
?>
            ...
            $('#ff').form('submit', {
                ...
                success: function(data) {
                    ...
                    if (arrData[0] == "GAGAL") {
                        ...
                        $.messager.alertLink('Info', arrData[1], 'info', '', 'app/loadUrl/app/konfigurasi_banner_add/?id=' + arrData[2]);
                    }
                }
            });
        }
    ...
    <!-- <script type="text/javascript" language="javascript" class="init">
        $(document).ready(function() {
            $('#btnMaps').on('click', function() {
                top.parent.openPopup("app/loadUrl/app/maps.php");
            });
        });

        function getDataMaps(long, lat) {
            $("#reqLongitude").val(long);
            $("#reqLatitude").val(lat);
        }
    </script> -->
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="judul-halaman">
                    <a href="app/loadUrl/app/konfigurasi_banner.php">Banner</a>
                    &bull; Form Banner
                </div>
                <div class="area-form">
                    <div class="konten">
                        <form id="ff" class="easyui-form form-horizontal" method="post" novalidate enctype="multipart/form-data">
                            ...
                            <div class="form-group">
                                ...
                                <div class='col-md-10'>
                                    <div class='form-group'>
                                        <div class='col-md-6'>
                                            <input class="easyui-textbox col-md-4" type="text" name="NAMA" value="<?= $NAMA ?>" data-options="required:true" style="width:100%">
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        <div class="col-md-12">
                                            <select name="STATUS" class="easyui-combobox" id="STATUS" required>
                                                <option <?= $STATUS == 'TAMPIL' ? 'selected' : '' ?> value="TAMPIL">Tampil</option>
                                                <option <?= $STATUS == 'TIDAK_TAMPIL' ? 'selected' : '' ?> value="TIDAK_TAMPIL">Tidak Tampil</option>
                                            </select>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="form-group">
                                <label for="KETERANGAN" class="control-label col-md-2">File Gambar</label>
                                <div class="col-md-9">
                                    <div class="form-group">
                                        <div class="col-md-11">
                                            <input name="GAMBAR" type="file" class="with-preview maxsize-21000" accept=".jpg" value="" 
                                            <? if ($GAMBAR == "") { ?> required <? } ?> />
    
                                            <input type="hidden" name="GAMBARTemp" value="<?= $GAMBAR ?>" />
                                            <?
                                            if (file_exists('uploads/banner/'. $GAMBAR)) {
                                            ?>
                                                <br><a href="uploads/banner/<?= $GAMBAR ?>" target="_blank"><?= $GAMBAR ?></a>
                                            <?
                                            }
                                            ?>
                                            <br><span class="text-muted">Max. ukuran file adalah 20MB, ekstensi .jpg</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
