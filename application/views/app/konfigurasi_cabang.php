    ...
    <script type="text/javascript" language="javascript" class="init">
        $(document).ready(function() {
            ...
            oTable = $('#example').dataTable({
                ...
                "bProcessing": true,
                "bServerSide": true,
                "sAjaxSource": "cabang_json/json/?reqToken=<?=$csrf->getToken()?>&PERUSAHAAN_ID=<?=$PERUSAHAAN_ID?>&STATUS=<?=$STATUS?>",
                columnDefs: [{
                    className: 'never',
                    targets: [0, 1, 2]
                }],
            ...
            $('#btnAdd').on('click', function() {
                document.location.href = 'app/loadUrl/app/konfigurasi_cabang_add';
            });
            ...
            $('#btnEdit').on('click', function() {
                ...
                document.location.href = 'app/loadUrl/app/konfigurasi_cabang_add/?id=' + anSelectedId;
            });
            ...
            $('#btnDelete').on('click', function() {
                ...
                deleteData("cabang_json/delete", anSelectedId, "app/loadUrl/app/konfigurasi_cabang/?STATUS="+$('#STATUS'). combobox('getValues'));
            });
            ...
            $('#btnAktif').on('click', function() {
                ...
                konfirmasiData("cabang_json/aktif", anSelectedId, "Apakah anda ingin mengaktifkan data?", "app/loadUrl/app/konfigurasi_cabang/?STATUS="+$('#STATUS'). combobox('getValues'));
            });
            ...
            $('#btnNonAktif').on('click', function() {
                ...
                konfirmasiData("cabang_json/non_aktif", anSelectedId, "Apakah anda ingin menonaktifkan data?", "app/loadUrl/app/konfigurasi_cabang/?STATUS="+$('#STATUS'). combobox('getValues'));
            });

            $('#PERUSAHAAN_ID').combobox({
                onSelect: function(param) {
                    oTable.fnReloadAjax("cabang_json/json/?reqToken=<?=$csrf->getToken()?>&PERUSAHAAN_ID="+$('#PERUSAHAAN_ID').combobox('getValues')+"&STATUS="+$('#STATUS').combobox('getValues'));
                }
            });
...
<body>
    <div class="container-fluid">
        <div class="row">
            <div class="col-md-12">
                <div class="judul-halaman">Cabang</div>
                <div class="konten">
                    <div class="area-menu-aksi">
                        ...
                        <input type="text" name="PERUSAHAAN_ID" class="easyui-combobox" id="PERUSAHAAN_ID" 
                        data-options="width:'400',editable:false, valueField:'id',textField:'text',url:'perusahaan_json/combo'" 
                        value="<?= $PERUSAHAAN_ID ?>" required />
                        <label class="ms-3">Status :</label>
                        <input name="STATUS" class="easyui-combobox" id="STATUS" 
                        data-options="width:'120',panelHeight:'100',panelWidth:'120',editable:false,valueField:'id',textField:'text',url:'combo_json/comboStatusAktif'" value="<?= $STATUS ?>" required />
                    </div>
                    <section>
