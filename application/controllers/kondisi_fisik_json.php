...
class kondisi_fisik_json extends CI_Controller
{
	...
	function json()
	{
		...
		$this->load->model("KondisiFisik");
		$kondisi_fisik = new KondisiFisik();

		$STATUS = $this->input->get("STATUS");

		$aColumns		= array("KONDISI_FISIK_ID","STATUS","KODE","NAMA","KETERANGAN","STATUS_KET");
		$aColumnsAlias	= array("KONDISI_FISIK_ID","STATUS","KODE","NAMA","KETERANGAN","STATUS_KET");
		...
		if (isset($_GET['iSortCol_0'])) {
			...
			if (trim($sOrder) == "ORDER BY KONDISI_FISIK_ID asc") {
				/*
		...
		$allRecord = $kondisi_fisik->getCountByParamsMonitoring(array(), $statement_privacy . $statement);

		if ($_GET['sSearch'] == ""){
			$allRecordFilter = $allRecord;
		}
		else{
			$allRecordFilter =  $kondisi_fisik->getCountByParamsMonitoring(array(), $statement);
		}

		$kondisi_fisik->selectByParamsMonitoring(array(), $dsplyRange, $dsplyStart, $statement, $sOrder);
		// echo $kondisi_fisik->query;exit;

		if($this->IS_MOBILE == true)
		{
			$arrResult["rowCount"] = $kondisi_fisik->rowCount;
			$arrResult["rowResult"] = $kondisi_fisik->rowResult;
		...
		while ($kondisi_fisik->nextRow()) {
			$row = array();
			for ($i = 0; $i < count($aColumns); $i++) {
				if($aColumns[$i] == "STATUS_KET") {
					if($kondisi_fisik->getField("STATUS") == "AKTIF"){
						$badge_color = "badge-success";
					}
					else{
						$badge_color = "badge-danger";
					}
					$row[] = "<span class='badge ".$badge_color."'>".str_replace("_", " ", $kondisi_fisik->getField("STATUS"))."</span>";
				}else{
					$row[] = $kondisi_fisik->getField($aColumns[$i]);
				}
			}
			$output['aaData'][] = $row;
		}
	...
	function add()
	{
		...
		/******* START VALIDASI ******/
		if($mode == "insert") {
			$adaKode = $kondisi_fisik->getCountByParams(array("KODE"=>$KODE));
			...
			$adaKode = $kondisi_fisik->getCountByParams(array("KODE"=>$KODE,"NOT KONDISI_FISIK_ID"=>$id));
		...
		$kondisi_fisik->setField("KONDISI_FISIK_ID", $id);
		$kondisi_fisik->setField("KODE", $KODE);
		$kondisi_fisik->setField("NAMA", $NAMA);
		$kondisi_fisik->setField("KETERANGAN", $KETERANGAN);
		$kondisi_fisik->setField("CREATED_BY", $this->USER_LOGIN_ID);
		$kondisi_fisik->setField("UPDATED_BY", $this->USER_LOGIN_ID);

		if($mode == "insert") {
			if($kondisi_fisik->insert()){
				$id = $kondisi_fisik->id;

				$KODE = "KF".generateZero($id, 6);

				$kondisi_fisik = new KondisiFisik();
				$kondisi_fisik->setField("KONDISI_FISIK_ID", $id);
				$kondisi_fisik->setField("FIELD", "KODE");
				$kondisi_fisik->setField("FIELD_VALUE", $KODE);
				if($kondisi_fisik->updateByField()){
	...
	function aktif()
	{
		...
		$kondisi_fisik->setField("KONDISI_FISIK_ID", $reqId);
		$kondisi_fisik->setField("FIELD", "STATUS");
		$kondisi_fisik->setField("FIELD_VALUE", "AKTIF");
		if ($kondisi_fisik->updateByField()){
	...
	function non_aktif()
	{
		...
		$kondisi_fisik->setField("KONDISI_FISIK_ID", $reqId);
		$kondisi_fisik->setField("FIELD", "STATUS");
		$kondisi_fisik->setField("FIELD_VALUE", "TIDAK_AKTIF");
		if ($kondisi_fisik->updateByField()){
	...
	function combo()
	{
		...
		$i = 0;
		$kondisi_fisik->selectByParams(array("STATUS" => "AKTIF"),-1,-1,$statement);
		// echo $kondisi_fisik->query;exit;
		while ($kondisi_fisik->nextRow()) {
			$arr_json[$i]['id']		= $kondisi_fisik->getField("KONDISI_FISIK_ID");
			$arr_json[$i]['text']	= $kondisi_fisik->getField("NAMA");
			$arr_json[$i]['KODE']	= $kondisi_fisik->getField("KODE");
			$arr_json[$i]['NAMA']	= $kondisi_fisik->getField("NAMA");
			$i++;
		}
