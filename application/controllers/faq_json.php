...
class faq_json extends CI_Controller
{
	...
	function json()
	{
		...
		$aColumns		= array("FAQ_ID","STATUS","PERTANYAAN","JAWABAN","STATUS_KET");
		$aColumnsAlias	= array("FAQ_ID","STATUS","PERTANYAAN","JAWABAN","STATUS_KET");
		...
		if (isset($_GET['iSortCol_0'])) {
			...
			if (trim($sOrder) == "ORDER BY FAQ_ID asc") {
				/*
		...
		$statement .= " AND (UPPER(A.PERTANYAAN) LIKE '%" . strtoupper($_GET['sSearch']) . "%')";

		$allRecord = $faq->getCountByParamsMonitoring(array(), $statement_privacy . $statement);

		if ($_GET['sSearch'] == ""){
			$allRecordFilter = $allRecord;
		}
		else{
			$allRecordFilter =  $faq->getCountByParamsMonitoring(array(), $statement);
		}

		$faq->selectByParamsMonitoring(array(), $dsplyRange, $dsplyStart, $statement, $sOrder);
		// echo $faq->query;exit;

		if($this->IS_MOBILE == true)
		{
			$arrResult["rowCount"] = $faq->rowCount;
			$arrResult["rowResult"] = $faq->rowResult;
		...
		while ($faq->nextRow()) {
			...
			for ($i = 0; $i < count($aColumns); $i++) {
				if($aColumns[$i] == "STATUS_KET") {
					if($faq->getField("STATUS") == "AKTIF"){
						$badge_color = "badge-success";
					}
					else{
						$badge_color = "badge-danger";
					}
					$row[] = "<span class='badge ".$badge_color."'>".str_replace("_", " ", $faq->getField("STATUS"))."</span>";
				}else{
					$row[] = $faq->getField($aColumns[$i]);
				}
			}
			$output['aaData'][] = $row;
		}
	...
	function add()
	{
		...
		$faq->setField("FAQ_ID", $id);
		$faq->setField("PERTANYAAN", $PERTANYAAN);
		$faq->setField("JAWABAN", $JAWABAN);
		$faq->setField("CREATED_BY", $this->USER_LOGIN_ID);
		$faq->setField("UPDATED_BY", $this->USER_LOGIN_ID);
	...
	function aktif()
	{
		...
		$faq->setField("FAQ_ID", $reqId);
		$faq->setField("FIELD", "STATUS");
		$faq->setField("FIELD_VALUE", "AKTIF");
		if ($faq->updateByField()){
	...
	function non_aktif()
	{
		...
		$faq->setField("FAQ_ID", $reqId);
		$faq->setField("FIELD", "STATUS");
		$faq->setField("FIELD_VALUE", "TIDAK_AKTIF");
		if ($faq->updateByField()){
	...
	function combo()
	{
		...
		$i = 0;
		$faq->selectByParams(array("STATUS" => "AKTIF"),-1,-1,$statement);
		// echo $faq->query;exit;
		while ($faq->nextRow()) {
			$arr_json[$i]['id']		= $faq->getField("FAQ_ID");
			$arr_json[$i]['text']	= $faq->getField("PERTANYAAN");
			$i++;
		}
