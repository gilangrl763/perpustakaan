  ...
  ***/
  ...
  function Entity()
	...
	public function getToken($idPegawai,$filter=""){
		$str = "SELECT MD5('".$idPegawai."'||'Pu12chasingLogin'||TO_CHAR(CURRENT_TIMESTAMP, 'HH24:MI')) AS JUMLAH    ";
	...
	public function getTokeRekanan($idPegawai,$filter=""){
		$str = "SELECT MD5('".$idPegawai."'||'Pu12chasingRekanan'||TO_CHAR(CURRENT_TIMESTAMP, 'HH24:MI')) AS JUMLAH   ";
		$query = $this->db->query($str);
		$row = $query->first_row();
		$temp = $row->jumlah; //next key*/
	...
	public function getNextId($idName,$tableName,$filter=""){
		$str = "SELECT MAX($idName) AS JUMLAH FROM $tableName ";
		$query = $this->db->query($str);
		$row = $query->first_row();
		$temp = $row->jumlah + 1; //next key*/
